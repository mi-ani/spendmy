    ...
    <section class="text-gray-600 body-font">
        <div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
            <div class="flex flex-row">
                <h1 class="sm:text-3xl text-2xl font-semibold title-font text-gray-900">Создание категории</h1>
            </div>
        </div>
    </section>
    {{--  Form block  --}}
                    ...
                    <form action="{{ route('category.store') }}" method="post">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <label class="block">
                                ...
                                <input type="text" name="name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="">
                            </label>
                            <label class="block">
                                ...
                                <div class="flex flex-row py-2">
                                    @foreach($colorList as $color)
                                        <div class="w-10 h-10 rounded-full overflow-hidden m-1 border-red-400"
                            ...
                            <label class="block">
                                ...
                                <div class="flex flex-row flex-wrap py-2">
                                    @foreach($iconList as $icon)
                                        <div class="w-10 h-10 rounded-full m-1 overflow-hidden border-red-400"
                                             style="background: grey" data-icon="{{ $icon->id }}" onclick="setIcon(this)">
                                ...
                                <div class="mt-2">
                                    <div>
                                        <label class="inline-flex items-center">
                                            <input type="checkbox" name="is_expense" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-offset-0 focus:ring-indigo-200 focus:ring-opacity-50" checked>
                            ...
                            <button type="submit" class="flex ml-auto text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">Создать</button>
                        </div>
                    </form>
                </div>
            </div>
